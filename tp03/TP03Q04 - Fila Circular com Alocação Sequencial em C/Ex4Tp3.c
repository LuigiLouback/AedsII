...
Personagem array[7];   // Elementos da pilha 
...
void inserir(Personagem x) {
    ...
    if (((ultimo + 1) % 6) == primeiro) {
        // Se a fila estiver cheia, remova o elemento mais antigo antes de inserir um novo
        Personagem tmpp = remover();
    }
    ...
    // Calcula a média após inserir o novo elemento
    int somaa = media();
