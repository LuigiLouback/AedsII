...
void selecao_recursiva(Personagem *array, int n, int i) {
    if (i >= n - 1)
        return;
    ...
    selecao_recursiva(array, n, i + 1);
}
bool pesqBin(Personagem *vet, int tamanho, char* x) {
    bool resp = false;
    int dir = tamanho - 1, esq = 0, meio;

    while (esq <= dir && !resp) {
        meio = (dir + esq) / 2;
        int comparacao = strcmp(x, vet[meio].name);
        if (comparacao == 0) {
            resp = true;
...
int n=0;
   ...
   //selecao_recursiva(persoangens,n,0);
   bool resp=false;
char entrada2[100];

//fgets(entrada2, sizeof(entrada2), stdin);
//entrada2[strcspn(entrada2, "\n")] = '\0'; // Remover o caractere de nova linha
scanf(" %[^\n]", entrada2);
while (!isFim(entrada2)) {
    bool resp = false;
    for(int i = 0; i < n; i++) {
        if(strcmp(persoangens[i].name, entrada2) == 0) {
            resp = true;
            break;
        }
    }
