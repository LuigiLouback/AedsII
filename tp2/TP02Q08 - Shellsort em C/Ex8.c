...
void insercaoPorCor(Personagem *array, int n, int cor, int h,int*numcompemovi){
    for (int i = (h + cor); i < n; i+=h) {
        Personagem tmp = array[i];
        int j = i - h;
        while ((j >= 0) ) {
            int comparacao =strcmp(array[j].eyeColour,tmp.eyeColour);
             int aux = numcompemovi[0]; 
        aux++;
        numcompemovi[0] = aux;
            if (comparacao == 0) {
                comparacao = strcmp(array[j].name, tmp.name);
            }
            if(comparacao>0){
                array[j + h] = array[j];
            j-=h;
...
//=============================================================================
void shellsort(Personagem *array, int n,int* numcompemovi) {
    int h = 1;
    ...
    do {
        ...
        for(int cor = 0; cor < h; cor++){
            insercaoPorCor(array, n, cor, h,numcompemovi);
        }
    } while (h != 1);
}
...
int main() {
    ...
    shellsort(persoangens, n, numcompemovi);
    for (int i = 0; i < n; i++) {
