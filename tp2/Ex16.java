 ...
 class Personagens {
  ...
  public class Ex16{
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i < nomesInseridos.length; i++) {
            Personagens chave = nomesInseridos[i];
            int j = (i < 10)?i-1:10-1;//parcial 
            
            while (j >= 0 && nomesInseridos[i] != null) {
                // Compara as datas de nascimento
                int comparacaoData = nomesInseridos[j].getDateOfBirth().compareTo(chave.getDateOfBirth());
                
                // Se as datas forem iguais, compara pelos nomes
                if (comparacaoData == 0) {
                    comparacaoData = nomesInseridos[j].getName().compareTo(chave.getName());
                }
