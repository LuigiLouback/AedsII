...
void naofuncionou(Personagem personagens[], int n) {
    int i, j;
    Personagem temp;
    for (i = 0; i < n-1; i++) {
        for (j = 0; j < n-i-1; j++) {
            if (strcmp(personagens[j].id, personagens[j+1].id) > 0) {
                temp = personagens[j];
...
//=============================================================================
Personagem getMax(Personagem *array, int n, int *numcompemovi) {
...
//=============================================================================
void radixsort(Personagem *array, int n, int *numcompemovi) {
    Personagem max = getMax(array, n, numcompemovi); 
    for (int exp = 1; (max.id[0] - '0') / exp > 0; exp *= 10) { 
        radcountingSort(array, n, exp, numcompemovi); 
    }
}

 void radcountingSort(Personagem *array, int n, int exp, int *numcompemovi) {
    int count[10] = {0}; // Inicializando o array de contagem como inteiros

    // Contando o número de ocorrências de cada dígito
    for (int i = 0; i < n; i++) {
        int digit = (array[i].id[0] - '0') / exp % 10; // Convertendo o primeiro dígito de id em um número inteiro
        count[digit]++;
    }

    // Transformando count no índice correto onde cada elemento deverá ser colocado no array de saída
    for (int i = 1; i < 10; i++) {
        count[i] += count[i - 1];
    }
