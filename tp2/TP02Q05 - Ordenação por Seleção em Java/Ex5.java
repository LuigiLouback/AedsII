 ...
 class Personagens {
  ...
  public class Ex5{
    ...
    public static void main(String[] args) {
        ...
        int numcomp=0;
        int nummovi=0;
        for(int i=0;i<(nomesInseridos.length-1);i++){
            menor=i;
            for(int j=i+1;j<nomesInseridos.length;j++){
                //comparando os nomes e ordenando em ordem alfabética
                if (nomesInseridos[j] != null) {
                comparacao=nomesInseridos[menor].getName().compareTo(nomesInseridos[j].getName());
                numcomp++;//numero de comparaçoes
                if(comparacao>0){
                    menor=j;
                }
            }}
            Personagens temp=nomesInseridos[menor];
            nomesInseridos[menor]=nomesInseridos[i];
            nomesInseridos[i]=temp;
            nummovi++;//numero de movimentações
        }
